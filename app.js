...
function resetWitnessUI() {
  const witnessScreen = document.getElementById('s-witness');
  if (witnessScreen) {
    witnessScreen.classList.remove('active');
    witnessScreen.style.display = 'none';
    witnessScreen.style.opacity = '0';
  ...
  ['decArrivalLine','decArrivalSub','decTapHint'].forEach(id => {
  ...
  ['bodymapWrap','voice-sense-layer','bodymap-tap-hint'].forEach(id => {
    const el = document.getElementById(id);
    if (el && el.parentNode) el.parentNode.removeChild(el);
  });
}
function restoreWitnessUIVisibility() {
  const witnessScreen = document.getElementById('s-witness');
  if (witnessScreen) {
    witnessScreen.style.display = '';
...
function goHome() {
  ...
  showScreen('s-home', () => {
    document.querySelectorAll('.screen').forEach(scr => { if (scr.id !== 's-home') scr.classList.remove('active'); });
    resetWitnessUI();
